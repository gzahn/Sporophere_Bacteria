...
invam <- readRDS(
  "./data/physeq_objects/clean_invam_bulk_soil_physeq_object.RDS"
)

# plot overall trait data for our taxa

# list of taxa/Nstrains
m <- paste(ps@sam_data$genus, ps@sam_data$species, ps@sam_data$isolate) %>%
  unique %>%
  sort() %>%
  str_split(" ")
paste(map_chr(m, 1), map_chr(m, 2)) %>% table %>% as.data.frame() %>% arrange(.)

x <- ps@sam_data %>%
  as('data.frame') %>%
  dplyr::select('order', starts_with('trait_')) %>%
  unique.data.frame()

GGally::ggpairs(
  x %>% dplyr::select(-order),
  color = 'gray30',
  fill = 'gray30'
) +
  theme_bw() +
...
y <- read_csv(
  "./data/metadata/DataRecord_1_CalculatedTraitMetrics_18Jun2024.csv"
) %>%
  dplyr::select(contains("_")) %>%
  mutate(source = "TraitAM")
names(y) <- paste0("trait_", names(y))
x %>%
  dplyr::select(-order) %>%
  mutate(trait_source = "This study") %>%
  full_join(y) %>%
  GGally::ggpairs(
    mapping = aes(color = trait_source),
    columns = c(
      "trait_vol_mean",
      "trait_shape_median",
      "trait_color_most",
...
# how many strains per taxon?
# distribution of traits in our taxa?
...
for (iso in levels(ps@sam_data$isolate)) {
  ...
  if (any(invam@sam_data$isolate == iso)) {
    ...
    x.sterile <-
      x.sterile %>%
      subset_taxa(
        taxa_sums(x.sterile) > 0 & taxa_names(x.sterile) %ni% taxa_names(y)
      ) # remove soil taxa from internal
...
x <- isolate_unsterile_taxa_ps$`MG101B-12` %>%
  microbiome::transform("compositional")
y <- isolate_unsterile_taxa_ps$`MG101B-12` %>%
  microbiome::transform("compositional") %>%
...
get_core_members <- function(x) {
  ...
  microbiome::core_members(
    x,
    detection = 0.2,
    prevalence = (nsamples(x) - 1) / nsamples(x),
    include.lowest = TRUE
  )
}
...
isolate_core_members <-
  data.frame(
    isolate = rep(names(isolate_core_ntaxa), isolate_core_ntaxa),
    asv = isolate_core_members %>% purrr::reduce(c)
  ) %>%
  mutate(taxonomy = corncob::otu_to_taxonomy(asv, ps)) %>%
  left_join(
    as(ps@sam_data, 'data.frame') %>%
      dplyr::select(isolate, starts_with("trait"))
  ) %>%
  unique.data.frame()
saveRDS(
  isolate_core_members,
  "./output/surface_bacteria_core_membership_by_isolate_df.RDS"
)
...
saveRDS(
  isolate_core_members2,
  "./output/internal_bacteria_core_membership_by_isolate_df.RDS"
)
...
# ...looks like this is suggesting that traits have strong influence on epi, but not really endo *core* communities

# should build predictions visualization

# should make ordination, colored by ornamentation height
ps
...
full@sam_data$mergevar <- paste(
  full@sam_data$g_spp,
  full@sam_data$location,
  sep = "|"
)
# Export
...
# repair metadata
full_m@sam_data$location <- sample_names(full_m) %>%
  str_split("\\|") %>%
  map_chr(2)
full_m@sam_data$g_spp <- sample_names(full_m) %>%
  str_split("\\|") %>%
  map_chr(1)
...
full_m@sam_data
ggsave(
  "./output/figs/phylum_composition_for_each_species_by_location.png",
  height = 10,
  width = 18
)
...
dat$Genus_species <- factor(
  dat$Genus_species,
  levels = sort(unique(dat$Genus_species))
)


p <-
  dat %>%
  ggplot(data = ., aes(x = isolate, y = richness, fill = location)) +
  geom_boxplot() +
...
p
ggsave(
  "./output/figs/species_richness_by_isolate_boxplot.png",
  height = 10,
  width = 18
)
